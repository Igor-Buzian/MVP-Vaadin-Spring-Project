...
public class RegistrationService {
    ...
    public ResponseEntity<?> createNewUser(RegisterDtoValues registerDtoValues, HttpServletResponse response, HttpServletRequest request){
        ...
        if(userPresenter.existsByEmail(registerDtoValues.getEmail())){
            ...
            if(!loginAttemptService.validateCaptcha(ip, registerDtoValues.getCaptchaResponse())) return ResponseEntity.status(HttpStatus.FORBIDDEN).header("Location", "/register?error=ip_banned").build();
            return ResponseEntity.status(HttpStatus.FORBIDDEN).header("Location","http://localhost:8081/register?error=exist_mail").build();
        }

        User user = new User();
        user.setEmail(registerDtoValues.getEmail());
        user.setEmail(registerDtoValues.getUsername());
        user.setPassword(passwordEncoder.encode(registerDtoValues.getPassword()));
          ...
          if(!registerDtoValues.getPassword().equals(registerDtoValues.getConfirmPassword()))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).header("Location","http://localhost:8081/register?error=different_password").build();
